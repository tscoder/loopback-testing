...
/**
 * [Originated from givenModel(), which take in array of attributes to
 * populate multiple models instead of one. After the test case, the models
 * will be destoryed in reverse order to allow simply dependencies between
 * these created models WITHIN THIS CALL (So other function call WILL NOT).
 *
 * Created by TSCoder.]
 *
 *
 * @param  {[type]} arr [Array of tuples. Each tuple takes in the following
 * parameter: modelName, attrs, and optionalHandler. See givenModel() for
 * details. ]
 *
 * @return {[Nil]}
 */
_beforeEach.givenModelsInOrder = function(arr) {
  var myarr = arr.map(function(tuple) {
    var result = {
      modelName: tuple[0],
      modelKey: tuple[0],
      attrs: tuple[1],
      optionalHandler: tuple[2]
    };
    if(typeof tuple[1] === 'function') {
      result.optionalHandler = result.attrs;
      result.attrs = undefined;
    }
    if(typeof tuple[2] === 'string') {
      result.modelKey = result.optionalHandler;
    }
  ...
  myarr.map(function(item) {
    beforeEach(function(done) {
      if(item.modelName === '__USERMODEL__') {
        item.modelName = this.userModel ? this.userModel : 'user';
      }

      var test = this;
      var app = this.app;
      var model = app.models[item.modelName];

      assert(model, 'cannot get model of name ' + item.modelName + ' from app.models');
      assert(model.dataSource, 'cannot test model '+ item.modelName +
        ' without attached dataSource');
      assert(typeof model.create === 'function',  item.modelName + ' does not have a create method');


      model.create(item.attrs, function(err, result) {
        if(err) {
          console.error(err.message);
          if(err.details) console.error(err.details);
          done(err);
        } else {
          test[item.modelKey] = result;
          done();
        }
      });
    });

    if(typeof item.optionalHandler === 'function') {
      beforeEach(item.optionalHandler);
    }
  });

  myarr.reverse().map(function(item) {
    afterEach(function(done) {
      this[item.modelKey].destroy(done);
    });
  });
}
...
_beforeEach.justLoggedInUser = function(credentials, optionalHandler) {
...
_describe.whenCalledRemotely = function(verb, url, data, cb) {
  ...
  // tscoder: added binary parser
  var need_bin = false;
  if (url.binary_content)
    need_bin = true;
  ...
  describe(verb.toUpperCase() + ' ' + urlStr, function() {
    beforeEach(function(cb) {
      ...
      if (need_bin) {
        // tscoder: Added binary parser
        this.http.parse(binaryParser);
      }
      if (data) {
        var payload = data;
        // tscoder: enable attachments when POST
        if (typeof data === 'object' && data.hasOwnProperty('attachments')) {
          var atta = data.attachments;
          if (typeof atta === 'object') {
            for (var ki in atta) {
              this.http.attach(ki, atta[ki]);
            }
          }
...
_describe.whenJustLoggedInAsUser = function(credentials, cb) {
  describe('when logged in as user (no creation)', function () {
    _beforeEach.justLoggedInUser(credentials);
    cb();
  });
}

_describe.whenJustCalledByUser = function(credentials, verb, url, data, cb) {
  describe('when called by logged in user (no creation)', function () {
    _beforeEach.justLoggedInUser(credentials);
    _describe.whenCalledRemotely(verb, url, data, cb);
  });
}
...
_it.shouldBeAllowedWhenJustCalledByUser =
function(credentials, verb, url, data) {
  _describe.whenJustCalledByUser(credentials, verb, url, data, function() {
    _it.shouldBeAllowed();
  });
}

_it.shouldBeDeniedWhenJustCalledByUser =
function(credentials, verb, url) {
  _describe.whenJustCalledByUser(credentials, verb, url, function() {
    _it.shouldBeForbidden();
  });
}
